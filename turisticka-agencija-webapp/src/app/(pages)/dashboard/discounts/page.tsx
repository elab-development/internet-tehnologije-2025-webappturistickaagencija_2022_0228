...
export default function DiscountsPage() {
  ...
  const refreshDiscounts = async () => {
    const res = await fetch("/api/discounts").then((r) => r.json());
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setArrId(""); setValue(""); setStartDate(""); setEndDate("");
      setShowForm(false);
  ...
  const startEdit = (d: Discount) => {
    ...
    setEditStart(d.startDate.split("T")[0]);
    setEditEnd(d.endDate.split("T")[0]);
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Da li ste sigurni da želite obrisati ovaj popust?")) return;
    ...
    if (res.ok) {
      ...
      setMessage("Popust obrisan.");
    } else {
      setMessage(data.message || "Greška.");
    }
  };

  const canEdit = (d: Discount) => {
    return user?.role === "ADMIN" || d.arrangement.createdById === user?.id;
  };

  const typeLabel = (t: string) => {
    return t === "PERCENTAGE" ? "Procenat (%)" : "Fiksni (€)";
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Popusti</h1>
          <p className="text-gray-500 mt-1">Upravljajte popustima na aranžmane</p>
        </div>
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5 w-full">
              ...
              >
                ...
                {arrangements.map((a) => (
                  <option key={a.id} value={a.id}>{a.destination}</option>
                ))}
              </select>
            </div>
            ...
            <InputField
              label={type === "PERCENTAGE" ? "Vrijednost (%)" : "Vrijednost (€)"}
              type="number"
              value={value}
      ...
      {discounts.length === 0 ? (
        <p className="text-gray-500">Nema popusta.</p>
      ) : (
              ...
              {editId === d.id ? (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <InputField
                      label={editType === "PERCENTAGE" ? "Vrijednost (%)" : "Vrijednost (€)"}
                      type="number"
                      value={editValue}
